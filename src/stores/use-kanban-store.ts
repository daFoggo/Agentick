...
export const useKanbanStore = create<KanbanState>()(
  persist(
    (set, get) => ({
      ...
      // Combined actions
      openCreateDialog: (statusId) => set({ isCreateOpen: true, defaultStatusId: statusId }),
      openEditDialog: (task) => set({ isEditOpen: true, selectedTask: task }),
      openDeleteDialog: (task) => set({ isDeleteOpen: true, taskToDelete: task }),
      closeDialogs: () =>
        set({
          isCreateOpen: false,
          isEditOpen: false,
      ...
      handleDragStart: (event, statuses) => {
        ...
        if (data?.type === "column") {
          set({ activeColumn: statuses.find((s) => s.id === active.id) || null })
      ...
      handleDragOver: (event) => {
        ...
        if (isOverColumn) {
          ...
          if (tasks[activeIndex] && tasks[activeIndex].status_id !== overTaskStatusId) {
            const newTasks = [...tasks]
            newTasks[activeIndex] = {
              ...newTasks[activeIndex],
      ...
      handleDragEnd: (event, statuses, projectId, initialTasks, updateStatusMutate, updateTaskMutate) => {
        const { active, over } = event
